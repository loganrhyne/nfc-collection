...
if [ ! -d "venv" ]; then
    ...
    echo "Please run pi-setup.sh first to create the virtual environment."
    exit 1
fi
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Usage: ./run-nfc-test.sh <script_number>"
    deactivate
    exit 1
fi
...
case "$1" in
    1)
        ...
        python testing-scripts/simple-reader.py
        ;;
    2)
        echo "Running Basic PN532 Test..."
        python testing-scripts/pn532-test.py
        ;;
    3)
        echo "Running Write Entry to NFC Tag..."
        python testing-scripts/write-entry-nfc.py
        ;;
    *)
